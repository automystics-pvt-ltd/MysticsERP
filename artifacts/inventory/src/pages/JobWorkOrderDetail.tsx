...
export default function JobWorkOrderDetail() {
  ...
  if (isLoading || !detail) {
    return (
      <div className="space-y-6">
        <PageHeader title="Job work order" />
        <div className="rounded-md border bg-card shadow-sm">
          <table className="w-full">
            <tbody>
              {Array.from({ length: 6 }).map((_, i) => (
                <tr key={i} className="border-b last:border-0">
                  <td className="p-4 w-40"><div className="h-4 w-24 rounded bg-muted animate-pulse" /></td>
                  <td className="p-4"><div className="h-4 w-48 rounded bg-muted animate-pulse" /></td>
                </tr>
              ))}
            </tbody>
          </table>
  ...
  const canEditRate = (!isCancelled && !isCompleted) && canEditPerm;
  ...
  const canEditInfo = (!isCancelled && !isCompleted) && canEditPerm;
