...
const CANCELLABLE_PURCHASE_STATUSES = ["draft", "ordered", "pending_approval"];
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <PageHeader
        ...
        breadcrumbs={[{ label: "Purchase Orders", href: "/purchase-orders" }, { label: order.orderNumber }]}
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <Can module="purchase_orders" action="approve">
          {order.status === "partially_received" && !isJobWorkBill && (
            <AlertDialog>
              <AlertDialogTrigger asChild>
                <Button
                  variant="outline"
                  disabled={shortCloseMutation.isPending}
                  data-testid="btn-short-close"
                >
                  <Scissors className="mr-2 h-4 w-4" /> Short-close
                </Button>
              </AlertDialogTrigger>
              <AlertDialogContent>
                <AlertDialogHeader>
                  <AlertDialogTitle>Short-close this order?</AlertDialogTitle>
                  <AlertDialogDescription>
                    This marks the order as fully received and waives the remaining undelivered quantities. No additional stock will be added. Use this when the supplier confirms the remaining items will not be shipped.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                  <AlertDialogCancel>Cancel</AlertDialogCancel>
                  <AlertDialogAction
                    onClick={() => shortCloseMutation.mutate(orderId)}
                    data-testid="btn-confirm-short-close"
                    disabled={shortCloseMutation.isPending}
                  >
                    Confirm Short-close
                  </AlertDialogAction>
      ...
      {order.status === "pending_approval" && (
        <ApprovalActions
          ...
          onApproved={() => queryClient.invalidateQueries({ queryKey: getGetPurchaseOrderQueryKey(orderId) })}
          onRejected={() => queryClient.invalidateQueries({ queryKey: getGetPurchaseOrderQueryKey(orderId) })}
        />
      )}
        ...
        <Card data-testid="card-goods-receipts">
          ...
          <CardContent className="space-y-4">
            {goodsReceipts.map((receipt) => {
              ...
              const stockConsumed = receipt.stockConsumed === true;
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        {!isCancelled && stockConsumed && (
                          <span
                            className="inline-flex items-center gap-1 text-xs font-medium text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 rounded px-1.5 py-0.5"
                            data-testid={`badge-stock-consumed-${receipt.id}`}
                          >
                      ...
                      <Can module="purchase_orders" action="approve">
                        {stockConsumed ? (
                          <Tooltip>
                            ...
                            <TooltipContent>
                              Cannot cancel — stock has been consumed or transferred out. Reverse those transactions first.
                            </TooltipContent>
                          </Tooltip>
                        ) : (
                          <AlertDialog>
