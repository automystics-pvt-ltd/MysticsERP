...
export default function StockTransferDetail() {
  ...
  const canTransferPerm = useCanI("stock_transfers", "transfer");
  const canDeletePerm = useCanI("stock_transfers", "delete");
  const canApprovePerm = useCanI("stock_transfers", "approve");
  ...
  const canDelete = status === "draft" && canDeletePerm;
    ...
    <div className="space-y-6">
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { label: transfer.fromWarehouseName, href: `/warehouses/${transfer.fromWarehouseId}?tab=transfers` },
      ...
      {status === "pending_approval" && (
        <ApprovalActions
          ...
          onApproved={() => queryClient.invalidateQueries({ queryKey: getGetStockTransferQueryKey(id) })}
          onRejected={() => queryClient.invalidateQueries({ queryKey: getGetStockTransferQueryKey(id) })}
        />
      )}
      ...
      {status !== "cancelled" ? (
        <div className="rounded-lg border bg-card px-6 py-4">
          <div className="flex items-center gap-0">
            {[
              ...
              const stepStatuses = ["draft", "in_transit", "completed"];
        ...
        <div className="rounded-lg border border-muted bg-muted/40 px-6 py-3 flex items-center gap-3">
          <XCircle className="h-5 w-5 text-muted-foreground shrink-0" />
          <div>
            <p className="text-sm font-medium text-muted-foreground">Transfer Cancelled</p>
            <p className="text-xs text-muted-foreground opacity-70">This transfer was cancelled and no longer affects stock.</p>
          </div>
        </div>
      )}
          ...
          <AlertDialogContent>
            ...
            <AlertDialogFooter>
              ...
              <AlertDialogAction
                ...
                disabled={completeMutation.isPending || cancelMutation.isPending || deleteMutation.isPending}
              >
