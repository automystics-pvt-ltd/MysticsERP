...
import {
  format, parseISO, subDays, startOfMonth, endOfMonth, subMonths,
} from "date-fns";
import { cn } from "@/lib/utils";
import type { DateRange as DayPickerRange } from "react-day-picker";

type DatePreset = "today" | "last_7" | "last_30" | "this_month" | "last_month" | "custom";
...
function detectPreset(from: string, to: string): DatePreset {
  const presets: Exclude<DatePreset, "custom">[] = ["today", "last_7", "last_30", "this_month", "last_month"];
      ...
      <PopoverContent className="w-auto p-0" align={align}>
        <div className="flex">
          <div className="flex flex-col gap-0.5 border-r p-2 min-w-[130px]">
            {(["today", "last_7", "last_30", "this_month", "last_month"] as const).map((p) => (
              <button
                key={p}
                type="button"
