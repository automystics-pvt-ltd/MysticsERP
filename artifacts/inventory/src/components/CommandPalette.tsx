...
const NAV_SHORTCUTS: Array<{
  ...
  { label: "Write-offs", href: "/write-offs", icon: ArrowLeftRight, keywords: ["damage", "expiry", "loss", "theft", "write off", "stock adjustment"] },
  ...
  { label: "Users & Roles", href: "/team", icon: UserCog, keywords: ["users", "members", "team", "roles", "permissions", "rbac"] },
...
function CommandPaletteContent({ onClose }: { onClose: () => void }) {
  ...
  const suppliers = useMemo(
    () => (suppliersQuery.data?.suppliers ?? []).slice(0, RESULT_LIMIT),
    [suppliersQuery.data],
  );
  ...
  const purchaseOrders = useMemo(() => {
    const q = debounced.toLowerCase();
    return (purchaseOrdersQuery.data?.orders ?? [])
      .filter(
        (po) =>
