...
router.patch("/organizations/current", async (req, res, next) => {
  try {
    ...
    if ("skuPrefix" in body) {
      ...
      if (raw == null || raw === "") {
        updates.skuPrefix = null;
      } else {
        const cleaned = String(raw)
          .toUpperCase()
          .replace(/[^A-Z0-9\-_]/g, "")
          .slice(0, 10);
        updates.skuPrefix = cleaned || null;
      }
    }
    if ("skuFormat" in body) {
      const fmt = String(body.skuFormat ?? "digits");
      updates.skuFormat = fmt === "alphanumeric" ? "alphanumeric" : "digits";
    }
    if ("skuNextNumber" in body) {
      const n = Number(body.skuNextNumber);
      if (Number.isInteger(n) && n >= 1) {
        updates.skuNextNumber = n;
      }
    }
    if ("defaultPaymentTermsDays" in updates) {
      const n = Number(updates.defaultPaymentTermsDays);
      if (!Number.isInteger(n) || n < 1 || n > 365) {
        res.status(400).json({ error: "defaultPaymentTermsDays must be a whole number between 1 and 365" });
        return;
      }
      updates.defaultPaymentTermsDays = n;
    }
