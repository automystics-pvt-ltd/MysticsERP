...
router.get("/job-work-orders", async (req, res, next) => {
  try {
    ...
    if (search) {
      conds.push(
        or(
          ilike(jobWorkOrdersTable.jwoNumber, `%${search}%`),
    ...
    const rows = await baseJwoQuery.limit(pageSize).offset((page - 1) * pageSize);
    ...
    const serializedOrders = rows.map((r) =>
      serializeOrder(
        r.o,
        r.supplierName,
        r.outputItemName,
    ...
    const countRows = await db
      ...
      .from(jobWorkOrdersTable)
      .innerJoin(suppliersTable, eq(suppliersTable.id, jobWorkOrdersTable.supplierId))
      .innerJoin(itemsTable, eq(itemsTable.id, jobWorkOrdersTable.outputItemId))
      .where(and(...conds));
    const total = Number(countRows[0]?.count ?? 0);
    res.json({ orders: serializedOrders, total, page, pageSize });
  } catch (err) {
    next(err);
  }
});
