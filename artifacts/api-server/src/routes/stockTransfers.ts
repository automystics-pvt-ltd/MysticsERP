...
router.get("/stock-transfers", async (req, res, next) => {
  try {
    ...
    if (typeof req.query.search === "string" && req.query.search.trim()) {
      ...
      conds.push(
        or(
          ilike(stockTransfersTable.transferNumber, s),
          sql`${sql.identifier(FROM_WH)}.name ILIKE ${s}`,
          sql`${sql.identifier(TO_WH)}.name ILIKE ${s}`,
        )!,
      );
    }
    // Optional server-side pagination.
    // When `page` or `pageSize` is present, respond with { transfers, total, page, pageSize }.
    // When absent (detail-page callers), respond with an array for backward compatibility.
    // Determined early so the itemId zero-result fast-path below can honour the same shape.
    const rawPage = req.query.page;
    const rawPageSize = req.query.pageSize;
    const paginate = rawPage !== undefined || rawPageSize !== undefined;
    const page = Math.max(1, Number(rawPage) || 1);
    const pageSize = Math.min(500, Math.max(1, Number(rawPageSize) || 25));
    ...
    if (intParams.itemId !== null) {
      ...
      if (ids.length === 0) {
        if (paginate) {
          res.json({ transfers: [], total: 0, page, pageSize });
        } else {
          res.json([]);
        }
        return;
    ...
    const fromJoin = sql`${warehousesTable} AS ${sql.identifier(FROM_WH)}`;
    const fromOn = sql`${sql.identifier(FROM_WH)}.id = ${stockTransfersTable.fromWarehouseId}`;
    const toJoin = sql`${warehousesTable} AS ${sql.identifier(TO_WH)}`;
    const toOn = sql`${sql.identifier(TO_WH)}.id = ${stockTransfersTable.toWarehouseId}`;
    ...
    if (paginate) {
      const [countRow] = await db
        ...
        .from(stockTransfersTable)
        .innerJoin(fromJoin, fromOn)
        .innerJoin(toJoin, toOn)
        .where(whereClause);
...
router.post("/stock-transfers/:id/dispatch", async (req, res, next) => {
  try {
    ...
    const result = await db.transaction(async (tx) => {
      ...
      // Approval workflow gate: if configured, stage dispatch for approval
      ...
      if (approvalResult) {
        // Store validated batch picks so the callback can apply them
        const batchPicksList: Array<{
          itemId: number;
          picks: { itemBatchId: number; quantity: number }[];
        }> = [];
        for (const [itemId, picks] of lineBatchPicks) {
          batchPicksList.push({
            itemId,
            picks: picks.map((p) => ({
              itemBatchId: p.itemBatchId,
              quantity: p.quantity,
            })),
          });
        }
