...
router.get("/customer-payments", async (req, res, next) => {
  try {
    ...
    if (req.query.search && typeof req.query.search === "string") {
      ...
      conds.push(
        or(
          ...
          ilike(customerPaymentsTable.referenceNumber, term),
        )!,
      );
    }
    ...
    // Omitting falls back to the legacy array shape for backward compat.
    ...
    if (rawPage !== null && !Number.isNaN(rawPage)) {
      ...
      const [countRows, rows] = await Promise.all([
        db
          ...
          .innerJoin(customersTable, eq(customersTable.id, customerPaymentsTable.customerId))
          .where(and(...conds)),
        db
          .select(selectShape)
          .from(customerPaymentsTable)
          .innerJoin(customersTable, eq(customersTable.id, customerPaymentsTable.customerId))
          .where(and(...conds))
          .orderBy(desc(customerPaymentsTable.paymentDate), desc(customerPaymentsTable.id))
          .limit(pageSize)
          .offset((page - 1) * pageSize),
      ]);
      res.json({
        payments: rows.map((r) => serializeCustomerPayment(r.payment, r.customerName)),
