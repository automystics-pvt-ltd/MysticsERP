  ...
  try {
    ...
    const rows = await db
      .select({
        ...
        isActive: organizationMembersTable.isActive,
  ...
  async (req, res, next) => {
    try {
      ...
      void writeAuditLog({
        ...
        description: `Invited ${b.email.toLowerCase()} as ${b.role}`,
  ...
  try {
    ...
    if (deleted[0]) {
      void writeAuditLog({
        ...
        description: `Revoked invitation for ${deleted[0].email}`,
        ipAddress: getClientIp(req),
      });
    }
    res.status(204).send();
...
router.patch(
  ...
  async (req, res, next) => {
    try {
      ...
      const result: TxResult = await db.transaction(async (tx: Tx) => {
      ...
      void writeAuditLog({
        ...
        description: `Changed ${u.email}'s role to ${m.role}`,
        changes: { role: { from: existing.role, to: m.role } },
        ipAddress: getClientIp(req),
      });
      res.json(
        serializeMember({
...
router.patch(
  ...
  async (req, res, next) => {
    try {
      ...
      const patch: Partial<{ canEditBills: boolean; canEditStocks: boolean; isActive: boolean }> = {};
      ...
      if (b.isActive !== undefined) {
        ...
        if (existing.role === "owner" && callerRole !== "owner") {
          res.status(403).json({ error: "Only owners can suspend another owner" });
          return;
        }
      ...
      if (b.isActive !== undefined && b.isActive !== existing.isActive) {
        void writeAuditLog({
          ...
          userId: t.userId,
          module: "team",
          action: b.isActive ? "activate" : "suspend",
          resourceType: "member",
          resourceId: String(m.id),
          description: `${b.isActive ? "Activated" : "Suspended"} access for ${u.email}`,
          changes: { isActive: { before: existing.isActive, after: b.isActive } },
          ipAddress: getClientIp(req),
        });
      }
      res.json(
...
router.post(
  ...
  async (req, res, next) => {
    try {
      const t = req.tenant!;
      if (!(await canManageTeam(t.organizationId, t.userId))) {
        res.status(403).json({ error: "Only owners or admins can reset passwords" });
        return;
      }
      const id = Number(req.params.id);
      const b = req.body as z.infer<typeof resetPasswordSchema>;
      ...
      if (member.userId === t.userId) {
        res.status(400).json({ error: "Use the change-password endpoint to update your own password" });
        return;
      }
      const callerRole = await getCallerRole(t.organizationId, t.userId);
      if (member.role === "owner" && callerRole !== "owner") {
        res.status(403).json({ error: "Only owners can reset another owner's password" });
        return;
      }
      ...
      await db
        .update(usersTable)
        // org-scope-allow: target user already verified to belong to this org
        // via the organizationMembersTable lookup above.
        .set({ passwordHash: hashed })
        .where(eq(usersTable.id, member.userId));

      const targetRows = await db
        .select({ email: usersTable.email })
        // org-scope-allow: fetching target user email for audit log only.
        .from(usersTable)
        .where(eq(usersTable.id, member.userId))
        .limit(1);
      const targetEmail = targetRows[0]?.email ?? "unknown";
      ...
      void writeAuditLog({
        ...
        description: `Reset password for ${targetEmail}`,
        ipAddress: getClientIp(req),
      });
...
router.delete("/team/members/:id", tenantMiddleware, async (req, res, next) => {
  try {
    ...
    const lastOwner = await db.transaction(async (tx: Tx) => {
      if (m.role === "owner") {
        const ownerRows = await tx.execute<{ id: number }>(sql`
          SELECT id FROM organization_members
           WHERE organization_id = ${t.organizationId}
    ...
    // Fetch user info for audit description (after confirmed delete)
    const removedUserRows = await db
    ...
    void writeAuditLog({
      ...
      resourceType: "member",
      resourceId: String(id),
      description: `Removed member ${removedUserRows[0]?.email ?? String(m.userId)} (${m.role})`,
      ipAddress: getClientIp(req),
    });
    res.status(204).send();
  } catch (err) {
  ...
  async (req, res, next) => {
    try {
      ...
      void writeAuditLog({
        ...
        action: "create",
        resourceType: "member",
        resourceId: String(result.member.id),
        description: `Created user ${result.user.email} as ${result.member.role}`,
        ipAddress: getClientIp(req),
      });
      res.status(201).json(
        serializeMember({
