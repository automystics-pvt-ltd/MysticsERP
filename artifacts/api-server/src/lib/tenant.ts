...
export interface TenantInfo {
  ...
  /** Resolved permission set — call can(module, action) to check. */
...
export async function ensureTenant(
  ...
  if (memberRows.length > 0) {
    ...
    if (requestedOrganizationId !== undefined) {
      ...
      if (!match) {
        ...
        if (userRow.isSuperAdmin) {
          ...
          const superPerms = await resolvePermissions(requestedOrganizationId, "owner");
          const superInfo: TenantInfo = {
            userId: userRow.id,
    ...
    const chosenRole = normalizeRole(chosen.role);
    const memberPerms = await resolvePermissions(chosen.organizationId, chosenRole);
    return {
      userId: userRow.id,
  ...
  if (requestedOrganizationId !== undefined && userRow.isSuperAdmin) {
    ...
    const saPerm2 = await resolvePermissions(requestedOrganizationId, "owner");
    return {
      userId: userRow.id,
      organizationId: requestedOrganizationId,
      role: "super_admin",
...
export async function tenantMiddleware(
  ...
  try {
    ...
    // Permission gate. Super admins bypass (they impersonate orgs and
    // need unrestricted access). For everyone else, check the resolved
    // permission set against the route policy table.
