...
//
// Each Vitest worker process gets its own dedicated per-worker database
// (`test_inv_w<VITEST_POOL_ID>`). On first import the public schema is
// dropped and recreated (DROP SCHEMA … CASCADE / CREATE SCHEMA) so the
// harness starts from a clean slate. The drizzle schema from `@workspace/db`
// is then pushed into the empty schema via `drizzle-kit/api`'s `pushSchema`.
//
// Using schema-level reset instead of DROP DATABASE / CREATE DATABASE is
// intentional: it works even when pool connections from a previous test run
// are still open, avoiding the race where pg_terminate_backend races with
// pool reconnects and causes DROP DATABASE to fail — which would leave stale
// objects in place and make pushSchema fail with "already exists" errors.
//
...
async function bootstrap(): Promise<void> {
  // ── Step 1: Attempt DROP DATABASE / CREATE DATABASE (clean-slate) ───────
  // The ideal path is a fully empty database — no diff, no ordering issues.
  // We try to terminate lingering connections first, then drop+recreate.
  // If DROP DATABASE fails (e.g. a pool from a concurrent test run reconnects
  // between the terminate sweep and the drop), we catch the error and fall
  // through to the schema-level reset in step 2, which is equally effective
  // and works even with active connections on the database.
  const admin = new pg.Client({ connectionString: adminUrl });
  await admin.connect();
  let droppedAndRecreated = false;
  try {
    await admin.query(
    ...
    // DROP DATABASE failed — a lingering connection got in the way.
    // Ensure the DB at least exists so the pool can connect.
    try {
      ...
      // If CREATE DATABASE also fails (DB already exists — the check above
      // had a TOCTOU race), that is fine; we proceed to schema reset below.
    }
  } finally {
    await admin.end();
  }

  // ── Step 2: Schema-level reset (always runs) ───────────────────────────
  // Even when step 1 succeeded, this normalises the public schema so that
  // pushSchema always sees a completely empty target regardless of which
  // path we took. It also handles pool reconnection: if the DB was just
  // drop+recreated, existing pool connections are on the deleted DB and will
  // error; pg.Pool discards them and reconnects to the new DB transparently.
  // DROP SCHEMA … CASCADE acquires per-object locks and succeeds even with
  // other connections open, so it is safe in both the fresh-DB and the
  // existing-DB paths.
  if (!droppedAndRecreated) {
    // Only needed when the DB wasn't fully recreated — the schema-level reset
    // clears stale objects that DROP DATABASE would have removed.
  ...
  // ── Step 3: Push the drizzle schema ────────────────────────────────────
  // With an empty public schema (either from DROP DATABASE or from the
  // schema reset above), pushSchema generates plain CREATE TABLE for every
  // table — no diff, no ALTER. result.apply() runs them in dependency order.
