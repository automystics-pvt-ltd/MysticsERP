...
describe("job-work-orders cross-tenant isolation", () => {
  ...
  describe("GET /job-work-orders (list + receipt totals)", () => {
    it("only returns the caller's own JWOs", async () => {
      ...
      const idsA = resA.body.orders.map((r: { id: number }) => r.id);
      ...
      const idsB = resB.body.orders.map((r: { id: number }) => r.id);
    ...
    it("the list-totals receipt rollup never crosses orgs", async () => {
      ...
      expect(resA.body.orders[0]).toMatchObject({
        id: a.jwoId,
        receivedQuantity: 5,
        scrappedQuantity: 0,
      });
      ...
      expect(resB.body.orders[0]).toMatchObject({
        id: b.jwoId,
        receivedQuantity: 5,
        scrappedQuantity: 0,
      });
