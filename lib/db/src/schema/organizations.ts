...
export const organizationsTable = pgTable(
  "organizations",
  {
    ...
    showMaxDiscountAmount: boolean("show_max_discount_amount").notNull().default(true),
    showMaxDiscountPercent: boolean("show_max_discount_percent").notNull().default(true),
    showOrderDiscount: boolean("show_order_discount").notNull().default(true),
    // ── SKU auto-generation ─────────────────────────────────────────
    // When skuMode='auto', the server generates a SKU on each POST /items
    // using: skuPrefix (optional) + skuNextNumber (zero-padded to 4 digits).
    // skuNextNumber is atomically incremented on every successful create so
    // concurrent inserts get unique values even without client coordination.
    // skuFormat is stored for future use (e.g. alphanumeric sequences).
    skuMode: text("sku_mode").notNull().default("manual"),
    skuPrefix: text("sku_prefix"),
    skuFormat: text("sku_format").notNull().default("digits"),
    skuNextNumber: integer("sku_next_number").notNull().default(1),
    // Default payment terms in days. Used to compute due dates when no explicit
