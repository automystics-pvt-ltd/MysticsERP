...
export const purchaseOrdersTable = pgTable(
  ...
  (t) => ({
    ...
    orgJwReceipt:   uniqueIndex("purchase_orders_org_jw_receipt_idx").on(t.organizationId, t.jobWorkReceiptId),
    // Filtering & sorting indexes
    orgStatusIdx:   index("purchase_orders_org_status_idx").on(t.organizationId, t.status),
    orgSupplierIdx: index("purchase_orders_org_supplier_idx").on(t.organizationId, t.supplierId),
    orgDateIdx:     index("purchase_orders_org_date_idx").on(t.organizationId, t.orderDate),
    orgCreatedIdx:  index("purchase_orders_org_created_idx").on(t.organizationId, t.createdAt),
  }),
);
